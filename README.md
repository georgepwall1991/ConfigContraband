...
A same-block `Configure<T>(GetSection(...))` / `GetRequiredSection(...)` paired with matching `AddOptions<T>().ValidateDataAnnotations()` is DataAnnotations-enabled, so nested graphs on that Configure registration are checked the same way. `Configure<T>` alone stays quiet — recursive attributes are meaningless until DataAnnotations run.
...
The diagnostic points at the JSON file, which a normal code fix cannot be anchored to, so the rename is offered as a refactoring on the C# registration instead: with the caret or selection on the `BindConfiguration(...)`/`Bind(...)` call (or on the bound-section literal), the refactoring menu lists `Rename key "WebookSecret" to "WebhookSecret" in appsettings.json` for every unknown key with a confident bindable-property match — one action per file containing the section. Keys that are legal binding aliases, keys without a confident suggestion, and sections projected from flat `"Stripe:Key"` pairs are skipped.
...
The same C#-anchored rename refactoring described under `CFG006` applies here: on the registration call, each strictly-failing key with a confident bindable-property match offers a rename in its JSON file.

### `CFG008`: Configuration Values That Cannot Bind To Their Target Type

The runtime `ConfigurationBinder` stores every configuration value as a string and converts it through `TypeDescriptor.GetConverter(type).ConvertFromInvariantString(value)` under the invariant culture. When a root property or direct read cannot convert — `"Port": "eighty"` for an `int`, `"Level": "Verbos"` for an enum, `"Enabled": "yes"` for a `bool` — the binder throws `InvalidOperationException` before your options ever validate. `CFG008` catches that at build time and points at the offending value in the `appsettings` file. What matters is the value's **string content, not its JSON kind**: `"Port": 8080` and `"Port": "8080"` both bind fine, while `"Port": true` (stored as the string `"True"`) does not.
...
- **Container elements:** arrays, binder-populatable collections, supported-key dictionaries, and dictionary collection values are checked only when the registration provably sets `ErrorOnUnknownConfiguration = true`. In that mode the runtime rethrows a malformed element conversion; loose binding catches the same exception and skips the bad item or entry, so CFG008 stays quiet. Unsupported dictionary keys, custom enumerable shapes, sets, open `object`/interface values, JSON `null`, and object-shaped scalar entries remain conservative.
- **Left alone (never reported):** `string`/`object` targets, JSON `null` (that is `CFG002`'s concern), and object/array values under a scalar-typed property when no supported nested target proves a conversion failure.
...
There is no automatic code fix — the diagnostic points at a JSON additional file rather than at C# the analyzer can rewrite. (`CFG006`/`CFG007` unknown keys do offer a rename refactoring from the C# registration; a bad value has no similarly safe rewrite.)
