...
internal static class JsonSchemaBuilder
{
    ...
    private static void ApplyConstraints(JsonObject schema, IPropertySymbol[] properties)
    {
        ...
        foreach (var attribute in effectiveAttributes.Values)
        {
            ...
            {
                case ValidationAttributeLimits.RangeAttributeName when kind == ScalarKind.Number:
                    ValidationAttributeLimits.ReadRange(attribute, ref minimum, ref maximum, ref minimumExclusive, ref maximumExclusive);
                    break;
                case ValidationAttributeLimits.MaxLengthAttributeName when kind == ScalarKind.String:
                    maxLength = ValidationAttributeLimits.StricterUpperBound(maxLength, ValidationAttributeLimits.ReadIntValue(attribute));
                    break;
                case ValidationAttributeLimits.StringLengthAttributeName when kind == ScalarKind.String:
                    maxLength = ValidationAttributeLimits.StricterUpperBound(maxLength, ValidationAttributeLimits.ReadStringLengthMaximum(attribute));
                    break;
