...
- Tightened review blockers so constructor/object-creation `BinderOptions` escapes disable strict proof, constructor-initialized get-only nested objects stay visible to validation diagnostics, constructor-assigned dictionary comparers are preserved, nested prepopulated polymorphic dictionary entries honor case-insensitive comparers, including `IEqualityComparer<string>` aliases, per dictionary level instead of applying ignore-case matching to the whole path, creatable reference collection/dictionary item CLR members accepted by strict binding do not warn, and `CFG006` remains visible when `CFG007` is disabled by analyzer configuration.
