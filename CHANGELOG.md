...
- Extended `CFG009` to the framework `configuration.Bind("key", instance)` overload, closing the last explicit binder-consumer path deferral. A constant keyed bind now reports when its absolute path is missing from visible appsettings files and the key is a near-miss of a declared sibling; plain misses remain quiet because environment providers may supply them. Root and known `GetSection(...)` chains, instance and static calls, and named arguments are supported. Review-driven guards require the instance argument to be provably side-effect free, preventing a stale warning when a helper or other effectful expression adds the path before binding executes. The shared direct-read normalization now verifies every `ConfigurationBinder` method through the real signed Microsoft assembly contract, so same-FQN source shadows remain quiet. Dynamic paths, stored sections with unknown origins, local/custom/mutated/escaped receivers, and unrelated inference boundaries are unchanged.

## 0.7.2 - 2026-07-16

- Extended `CFG008` to direct framework `ConfigurationBinder.GetValue<T>` reads, so a visible appsettings scalar that provably cannot convert to the requested type is reported before the runtime binder throws `InvalidOperationException`. Root paths and known `GetSection(...)` chains are supported, including static and named calls plus the default-value overload when its default is a compile-time constant. The rule reuses the existing host-configuration provenance and scalar-conversion boundaries, deduplicates repeated reads and diagnostics already produced through options binding, and verifies the invoked method belongs to the real signed Microsoft `Microsoft.Extensions.Configuration.Binder` assembly rather than a same-FQN source shadow or unsigned replacement assembly. Review-driven guards keep effectful or unprovable default expressions and interface-typed fields/properties with an unproven configuration origin quiet, preventing stale appsettings diagnostics after argument side effects or against locally built member configuration; constructor assignments that overwrite a `null!` DI placeholder are also treated as ambiguous, with same-file assignments bound to the actual member symbol so a shadowing parameter cannot suppress a valid diagnostic. Non-null cross-file partial-class member initializers and cross-file constructor writes are classified conservatively from syntax instead of requesting a second semantic model, avoiding both `AD0001` and Roslyn's `RS1030` analyzer-host warning. Missing paths, JSON `null`, non-scalar values, non-constant keys, stored `IConfigurationSection` receivers, local or mutated configuration, and custom providers also remain quiet. The CFG008 title and description now refer to the general target type rather than only an options property; its ID, severity, and message arguments are unchanged.

## 0.7.1 - 2026-07-16

- Fixed a `CFG009` false negative in receiver-provenance tracking: a safe same-block assignment or alias that pointed back to the host configuration contract stopped being recognized when the value was explicitly cast to `IConfiguration` (for example `current = (IConfiguration)configuration`), so a later missing `GetRequiredSection(...)` read was silently skipped. Provenance unwraps only non-user-defined configuration-interface conversions, while mutation rooting conservatively follows any non-user-defined cast. Casts of locally constructed `ConfigurationManager` values and concrete custom configuration providers remain quiet, including a downcast from the contract to a custom provider, and mutations through interface or concrete casts still suppress the diagnostic. Diagnostic IDs, severities, section-path resolution, and unrelated inference boundaries are unchanged.

## 0.7.0 - 2026-07-16

- Added **`CFG009`** (Warning): *Direct configuration path is unavailable from visible appsettings files.* Every existing rule saw configuration only through an options registration, but plenty of code reads `IConfiguration` directly — and a typo there is just as fatal and even quieter: `configuration.GetRequiredSection("Strpie")` throws `InvalidOperationException` at runtime, while `GetSection("Strpie").Get<ServerOptions>()` / `.Bind(instance)` silently binds nothing. `CFG009` extends the `CFG001` missing-section check — including the "Did you mean" suggestion and the code fix that rewrites the section literal — to direct reads: standalone `GetRequiredSection(...)` (chained paths, constant/`nameof` keys, `?.`/parenthesized/null-forgiving receivers, and host `IConfiguration`/`IConfigurationRoot`/`ConfigurationManager` contracts), `GetSection(...).Get<T>()` / `.Bind(instance)` binder consumers when the unavailable path is a near-miss of a declared sibling (plain misses stay quiet because environment providers may supply them), and `GetConnectionString(...)` gated on a provable typo (a `ConnectionStrings` section exists in appsettings and the name is a near-miss of a declared entry). The rule verifies the invoked method is the real `Microsoft.Extensions.Configuration` extension, classifies receiver provenance conservatively across same-block assignments and aliases, and stays silent for concrete custom providers, locally built configuration, receiver mutation/escape/capture, and unprovable paths. Reads that feed a recognized options registration stay with `CFG001`, and an unavailable `GetRequiredSection` chain reports once at the innermost unavailable parent. Section availability now matches the referenced JSON provider: on .NET 10, empty objects and explicit `null` are unavailable while empty arrays exist; version-sensitive shapes stay quiet when provider semantics are unknown. `GetValue<T>(...)`, the `configuration["key"]` indexer, and the `Bind("key", instance)` overload remain deliberately out of scope. The analyzer test references gained `Microsoft.Extensions.Configuration.Json` 10.0.0; shared missing-section reporting was extracted from `CFG001` unchanged.

## 0.6.0 - 2026-07-08

- Added **`CFG008`** (Warning): *Configuration value cannot be bound to the target property type.* A scalar value in a bound `appsettings` section that provably cannot convert to the bound property's CLR type — `"Port": "eighty"` for an `int`, `"Level": "Verbos"` for an enum, `"Enabled": "yes"` for a `bool` — now reports at the value's location, because the runtime `ConfigurationBinder` throws `InvalidOperationException` while binding it (before options validation ever runs). This fills the gap between `CFG001` (section exists), `CFG006`/`CFG007` (keys are known), and `CFG002` (required keys are present): nothing previously checked that a value is actually *convertible*. The check mirrors the binder's `TypeConverter` semantics — invariant culture, and the value's string content rather than its JSON kind (`"Port": 8080` and `"Port": "8080"` both bind; `"Port": true` does not). It fires only on a provable failure and stays conservative wherever invariant `TryParse` is stricter than the runtime converter: `#`/`0x` hex integers, empty strings for `char`/`DateTime`/`DateTimeOffset`, enum comma-lists and numeric enum values, and case-insensitive `bool` values are all accepted. Covered target types are the integral types, `float`/`double`/`decimal`, `bool`, `char`, enums, `Guid`, `TimeSpan`, `DateTime`, and `DateTimeOffset` (each unwrapped from `Nullable<T>`). `string`/`object` targets, JSON `null` (a `CFG002` concern), object/array values under scalar properties, and collection/dictionary *element* mismatches are intentionally left alone. There is no automatic code fix, since the diagnostic points at a JSON additional file. The appsettings model now records each scalar value's kind, text, and location; the generated `appsettings.schema.json` output is unchanged.

## 0.5.16 - 2026-07-07

- Fixed a `CFG007`/`CFG006` **false positive** found by evidence-based audit: `TryGetDictionaryValueType` only ever read the dictionary's *value* type, so any `Dictionary<TKey, TValue>` was treated as fully bindable regardless of its key type. The real `ConfigurationBinder`'s `BindDictionary`/`BindDictionaryInterface` only support `string`, enum, and integral (`sbyte` through `ulong`) key types — for any other key type (`Guid`, `double`, `bool`, `TimeSpan`, a custom struct, ...) it silently returns before binding anything, so it never evaluates `ErrorOnUnknownConfiguration` and never throws in strict or loose mode. `CFG007` still reported a typo'd key under a `Dictionary<Guid, EndpointOptions>` value even though the runtime binder never looks at it. A new `TryGetSupportedDictionaryValueType` gates every analyzer/metadata decision point that recurses into or reports on dictionary values (nested-object recursion, scalar/nested-dictionary strict reporting) so an unsupported-key dictionary is fully opaque — no `CFG006`/`CFG007` under it — while string-, enum-, and integral-keyed dictionaries keep exactly today's behavior. Guarded against a related reclassification trap: an unsupported-key dictionary also implements `IEnumerable<KeyValuePair<TKey, TValue>>`, so once its dictionary-specific handling is gated it must not fall through to the generic collection-of-`KeyValuePair` or plain-scalar strict-reporting branches either. The generated `appsettings.schema.json` output is unchanged — the schema builder deliberately keeps using the unfiltered `TryGetDictionaryValueType`, since schema nesting has always stayed independent of key-type binding rules. Diagnostic IDs, severities, and unrelated inference boundaries are unchanged.
...
- Fixed a shared `CFG005`/`CFG006` **false negative** found by evidence-based audit: struct- and record-struct-typed nested options properties were excluded from analysis because the nested-object gate filtered on `TypeKind.Class` only, even though the real `ConfigurationBinder` binds and recurses into struct-typed properties. As a result a typo under a struct-typed nested property went unflagged (`CFG006`), and a nested struct carrying validation attributes without recursive validation on the parent was skipped (`CFG005`). The gate now also accepts `TypeKind.Struct` (primitive and BCL value types such as `int`, `DateTime`, `Guid`, `Nullable<T>` stay excluded, and enums are unaffected). Consistently, `CFG002` now recurses into a `[ValidateObjectMembers]` struct property's members even when its section is absent, because a struct's `default(T)` is a non-null instance that DataAnnotations validates at runtime (a missing `[Required]` member throws) — previously the missing-section recursion used class/null default semantics and skipped it. The generated `appsettings.schema.json` output is unchanged — the schema builder deliberately keeps value-type properties permissive, since an unknown struct may be bound from a scalar via a `TypeConverter`. Diagnostic IDs, severities, and unrelated inference boundaries are unchanged.
