...
- Tightened review blockers so constructor/object-creation `BinderOptions` escapes disable strict proof, constructor-initialized get-only nested objects stay visible to validation diagnostics, constructor-assigned dictionary comparers are preserved, and nested prepopulated polymorphic dictionary entries honor case-insensitive comparers per dictionary level instead of applying ignore-case matching to the whole path.
