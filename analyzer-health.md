...
| CFG004 DataAnnotations not enabled | Warning | 4 | 5 | 5 | 4 | 5 | 5 | 4.60 | P3 | Covers root, inherited, nested, and type-level bindable DataAnnotations plus `IValidatableObject` on supported named and unnamed `OptionsBuilder<T>` bindings, including single-constructor-bound properties inherited from base options types, initialized get-only nested object/collection properties, and private-set properties when `BindNonPublicProperties` is enabled; recognizes the framework `ValidateDataAnnotations()` before and after binding in fluent and immediate same-block local chains, avoids duplicate `ValidateOnStart()` when startup validation already exists, and shares the named-chain-safe formatter appender with CFG003. |
