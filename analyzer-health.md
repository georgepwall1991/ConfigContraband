...
| CFG004 DataAnnotations not enabled | Warning | 4 | 5 | 5 | 4 | 5 | 5 | 4.60 | P3 | Covers root, inherited, and nested bindable DataAnnotations plus `IValidatableObject` on supported named and unnamed `OptionsBuilder<T>` bindings, including constructor-bound properties inherited from base options types, initialized get-only nested object/collection properties, and private-set properties when `BindNonPublicProperties` is enabled; recognizes the framework `ValidateDataAnnotations()` before and after binding in fluent and immediate same-block local chains, avoids duplicate `ValidateOnStart()` when startup validation already exists, and shares the named-chain-safe formatter appender with CFG003. |
| CFG005 Nested validation not recursive | Warning | 5 | 4 | 5 | 5 | 5 | 5 | 4.75 | P3 | Strong current shape. Covers recursive object and collection graphs, including constructor-bound nested objects inherited from base options types, initialized get-only object/collection properties, opt-in private-set nested properties, nested `IValidatableObject` types, and user namespaces that merely start with `System`, on supported `OptionsBuilder<T>` bindings; suppresses unsafe interface/uninitialized get-only/default-private-set cases and proves cross-document recursive-attribute fixes, including constructor-bound object and collection record parameters, inherited property declarations, namespace-local using handling, and local attribute-name conflicts. |
| CFG006 Unknown configuration key | Info | 4 | 4 | 5 | 5 | 5 | 5 | 4.50 | P3 | Broadest test depth. Covers named and unnamed `BindConfiguration(...)`, `Bind(GetSection(...))`, and direct `Configure<T>(GetSection(...))`; recurses through constructor-bound records/classes including inherited public properties, nested objects, initialized get-only objects, object collections, initialized get-only collections, dictionary values, dictionary values containing collections, explicit `BindNonPublicProperties`, comments, JSON string escapes, `[ConfigurationKeyName]` key overrides on property-bound options and settable constructor-bound properties, including opted-in private setters, when the constructor key is present or the constructor parameter has a default value, merged colon-delimited appsettings keys, dot-qualified appsettings file names, and user namespaces that merely start with `System` while keeping scalar arrays and dictionary entry names quiet. |
...
- Includes root, inherited, and nested public bindable properties, including constructor-bound properties inherited from base options types and initialized get-only object/collection properties, plus options types implementing `IValidatableObject`.
...
- Treats constructor-bound nested objects, including inherited public properties populated through a derived constructor, and initialized get-only object/collection properties as bindable, matching runtime binder behavior.
...
- Treats constructor-bound records/classes, including inherited public properties, and initialized get-only object, collection, and dictionary properties as bindable, while leaving uninitialized get-only properties outside the static bindable graph when no mapped constructor parameter exists.
