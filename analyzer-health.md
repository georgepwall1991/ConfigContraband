...
- Offers fixes for `[ValidateObjectMembers]` and `[ValidateEnumeratedItems]`, including options properties declared in a different source document from the registration diagnostic, constructor-bound record parameters that require a `property:` attribute target, files that use namespace-local using blocks, and files with same-named local attributes that require fully qualified attribute insertion.
