...
| CFG003 Validation not on startup | Warning | 4 | 5 | 5 | 4 | 5 | 5 | 4.60 | P3 | Good analyzer boundary for named and unnamed fluent or immediate same-block local `OptionsBuilder<T>` chains, including initializer-bound and later local binding statements plus `Bind(GetSection(...))`; tracks symbol-checked framework validation/startup calls before and after the binding call, honors `AddOptionsWithValidateOnStart<TOptions>()`, and code fixes preserve multiline formatting, comments, split locals, and single-line chains. |
