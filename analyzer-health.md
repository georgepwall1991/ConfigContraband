...
| CFG004 DataAnnotations not enabled | Warning | 4 | 5 | 5 | 4 | 5 | 5 | 4.60 | P3 | Covers root, inherited, and nested bindable DataAnnotations plus `IValidatableObject` on supported named and unnamed `OptionsBuilder<T>` bindings, including single-constructor-bound properties inherited from base options types, initialized get-only nested object/collection properties, and private-set properties when `BindNonPublicProperties` is enabled; recognizes the framework `ValidateDataAnnotations()` before and after binding in fluent and immediate same-block local chains, avoids duplicate `ValidateOnStart()` when startup validation already exists, and shares the named-chain-safe formatter appender with CFG003. |
| CFG005 Nested validation not recursive | Warning | 5 | 4 | 5 | 5 | 5 | 5 | 4.75 | P3 | Strong current shape. Covers recursive object and collection graphs, including single-constructor-bound nested objects inherited from base options types, initialized get-only object/collection properties, opt-in private-set nested properties, nested `IValidatableObject` types, and user namespaces that merely start with `System`, on supported `OptionsBuilder<T>` bindings; suppresses unsafe interface/uninitialized get-only/default-private-set/ambiguous-constructor cases and proves cross-document recursive-attribute fixes, including constructor-bound object and collection record parameters, inherited property declarations, namespace-local using handling, and local attribute-name conflicts. |
...
- Includes root, inherited, and nested public bindable properties, including single-constructor-bound properties inherited from base options types and initialized get-only object/collection properties, plus options types implementing `IValidatableObject`.
