...
- Tracks immediate same-block local `OptionsBuilder<T>` calls, including binding in the initializer or a later local expression statement, until an unrelated statement breaks the sequence. For later local binding statements, adjacent same-local calls before and after the bind are recognized.
